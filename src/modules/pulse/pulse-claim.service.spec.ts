...
describe('Pulse claim gate and confirmation', () => {
  const prisma = {
    ...
    pulseClaim: { findUnique: jest.fn(), create: jest.fn(), update: jest.fn(), count: jest.fn() },
  ...
  it('confirms a held claim before its hold expires', async () => {
    ...
    expect(prisma.pulseClaim.update).toHaveBeenCalledWith({ where: { id: 'c1' }, data: { state: 'confirmed', holdExpiresAt: null } });
  });
  ...
  it('refuses to confirm a claim whose hold already expired', async () => {
    ...
    expect(prisma.pulseClaim.update).not.toHaveBeenCalled();
  });
