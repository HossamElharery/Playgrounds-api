...
describe('WalletService', () => {
  ...
  beforeEach(() => {
    ...
    prisma.walletWithdrawalRequest.updateMany.mockResolvedValue({ count: 1 });
    prisma.walletWithdrawalRequest.findUniqueOrThrow.mockResolvedValue({
      id: 'w1',
      status: 'rejected',
    });
  ...
  describe('withdrawals', () => {
    ...
    it('refunds the wallet when an admin rejects the request', async () => {
      ...
      expect(prisma.walletWithdrawalRequest.updateMany).toHaveBeenCalledWith({
    ...
    it('does not touch the wallet when marking a withdrawal paid', async () => {
      prisma.walletWithdrawalRequest.findUnique.mockResolvedValue({ id: 'w1', userId: 'u1', amount: 10000, status: 'pending' });
      prisma.walletWithdrawalRequest.findUniqueOrThrow.mockResolvedValue({ id: 'w1', status: 'paid' });
    ...
    it('does not refund when another admin wins the resolution race', async () => {
      prisma.walletWithdrawalRequest.findUnique.mockResolvedValue({
        id: 'w1', userId: 'u1', amount: 10000, status: 'pending',
      });
      prisma.walletWithdrawalRequest.updateMany.mockResolvedValue({ count: 0 });

      await expect(
        service.resolveWithdrawal('admin2', 'w1', 'rejected'),
