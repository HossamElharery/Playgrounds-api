...
describe('Owner dashboard integrity', () => {
  ...
  it('rejects finance for another owner venue', async () => {
    await expect(service.finance('other','v1','2026-09-01','2026-09-07')).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('includes the whole final day and uses configured commission', async () => {
    ...
    const result = await service.finance('owner','v1','2026-09-01','2026-09-07');
  ...
  it('does not add different currencies together', async () => {
    ...
    await expect(service.finance('owner','v1','2026-09-01','2026-09-07')).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('revoking an invite also removes its venue access', async () => {
    ...
    await service.revokeStaffInvite('owner','invite');
  ...
  it('owner cannot accept a pending invitation on behalf of a staff member', async () => {
    ...
    await expect(service.setStaffStatus('owner','invite','accepted')).rejects.toBeInstanceOf(BadRequestException);
  });
});
